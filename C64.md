...
Las instrucciones `PHA` y `PHP` **meten** el contenido del registro en la pila, mientras que `PLA` y `PLP` **sacan** un byte de la pila y lo guardan en los registros. Si queremos consultar o modificar el **puntero** actual a la **pila**, tendremos que usar las instrucciones `TSX` y `TXS`.
