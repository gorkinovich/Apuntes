...
| Bit | Máscara | Serial Bus | Datasette | RS-232 |
|:---:|:-------:|------------|-----------|--------|
| 0 | 1   | En caso de *timeout*:<br/>0 = Lectura<br/>1 = Escritura | - | Error de paridad |
...
20 GET# 1,C$
30 PRINT C$;
40 IF NOT(STATUS AND 64) THEN 20
50 PRINT
60 CLOSE 1
```
...
Con `PEEK`, indicando la dirección de memoria que queremos consultar, obtendremos el byte almacenado en la dirección. Con `POKE` podremos almacenar un byte en una dirección de la memoria, es decir, *valor* admite desde `0` hasta `255`. Las direcciones se indican en base 10, ya que no tenemos en este lenguaje la posibilidad de indicar valores hexadecimales, y son enteros sin signo de 16 bits (desde `0` hasta `65536`).
...
El registro **A** se le conoce como el acumulador y el **P** contiene el estado de los flags de la CPU, para controlar el resultado de algunas operaciones. Estos registros serán explicados con más detenimiento en la sección del lenguaje ensamblador. Veamos un ejemplo de lo que podemos hacer:
