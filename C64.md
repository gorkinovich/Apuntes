...
Por defecto tenemos 43.007 bytes de memoria libre disponible, que se pueden ampliar a 51.200 bytes si desactivamos la ROM de BASIC.

## Interfaz gráfica

Por defecto, el buffer de pantalla está en las direcciones **\$0400-\$07E7** y el de color en **\$D800-\$DBE7**. El buffer de pantalla funciona con códigos de pantalla (distintos de los códigos PETSCII), que representan el índice dentro de la tabla de caracteres o tiles que tengamos cargada en memoria. Estos búferes son de **1000 bytes** ya que son una tabla de **40x25 celdas** (ancho x alto). Cada celda tiene un tamaño de **8x8 píxeles**.

Dentro del puerto A (**\$DD00**) de la CIA2, modificando los **bits 0-1**, podemos configurar cual de los cuatro bancos de memoria puede acceder el chip VIC-II, ya que este trabaja con regiones de 16 KB de memoria:
...
Esto define los bits 14 y 15 de las direcciones a las que va a acceder el chip VIC-II. Una vez seleccionado el banco, podemos desde el registro en **\$D018** configurar la ubicación de la memoria de caracteres/tiles, del bitmap y/o de la pantalla. Lo veremos en la sección de los modos de pantalla.
...
<p align="center"><img src="https://gorkinovich.github.io/learn/C64-Colors.png" alt="Fuentes del C64"></p>
...
### Modo de caracteres/tiles

El modo de caracteres/tiles es un grupo de modos gráficos que gestionan el buffer de pantalla indicando en cada celda el carácter que ha de contener. Con este modo se puede representar, gastando poca memoria, los niveles de un juego. Hay tres modos de caracteres: el estándar, el multicolor y el de color de fondo extendido.

Los tres modos implican una representación de la pantalla como una tabla de **40x25 celdas** (columnas x filas), para ello hay un buffer de pantalla, que contiene los índices a los caracteres, y un buffer de color para la pantalla. Por defecto, el buffer de pantalla está en la región **\$0400-\$07E7** y el de color en **\$D800-\$DBE7**, ambos dos tienen un tamaño de **1000 bytes**.

Cada celda de la pantalla es un byte, por lo tanto podemos disponer de 256 posibles valores que referenciar en el juego de caracteres. Cada carácter tiene unas dimensiones de **8x8 píxeles**, por lo tanto la resolución de la pantalla es de **320x200 píxeles**. Para definir un carácter hace falta 8 bytes, donde cada bit representa la columna de la fila correspondiente al byte, y para definir un juego de caracteres hace falta 8 x 256 = 2.048 bytes (2 KB).

El modo estándar es el que está activado por defecto al arrancar la máquina.

..

### Modo bitmap

El modo bitmap es un grupo de modos gráficos que gestionan el buffer de pantalla indicando pixel a pixel su contenido. Con este modo se puede representar imágenes gráficas más complejas que en la sección anterior, pero su coste de memoria es superior. Hay dos modos bitmap: el estándar y el multicolor.
